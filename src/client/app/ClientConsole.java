...
public class ClientConsole {
    ...
    private void employeeMenuLoop() throws IOException {
        while (true) {
            ...
            System.out.println("1) Sell (cart, one payment)");
            ...
            System.out.println("5) List inventory (my branch)");
            ...
            if ("0".equals(c)) {
                ...
                doSellCart(employeeBranch);
            } else if ("2".equals(c)) {
                doBuy(employeeBranch);
            } else if ("3".equals(c)) {
                doAddProduct(employeeBranch);
            } else if ("4".equals(c)) {
                doRemoveProduct(employeeBranch);
            } else if ("5".equals(c)) {
    ...
    private void stockManagementMenu() throws IOException {
        // (לא בשימוש עוד – השארנו לתאימות אם אתה קורא לזה ממקום אחר)
        while (true) {
            System.out.println("\n-- Stock Management --");
            System.out.println("1) Sell (cart, one payment)");
            System.out.println("2) Order product to branch");
            ...
            else if ("1".equals(c)) doSellCart(employeeBranch);
    ...
    private void startChatClient() {
        ...
        try (Socket chat = new Socket(HOST, CHAT_PORT);
            ...
            while (true) {
                ...
                String c = in.nextLine().trim();
                if ("0".equals(c)) { chatOut.println("QUIT"); break; }
    ...
    private void doSellCart(Branch branch) throws IOException {
        ...
        System.out.println("Enter items. Leave SKU blank to finish.");
        java.util.LinkedHashMap<String,Integer> cart = new java.util.LinkedHashMap<>();
        while (true) {
            System.out.print("SKU (blank = finish): ");
        ...
        while ((line = reader.readLine()) != null) {
            ...
            else if (line.startsWith("ERR ")) { System.out.println(line.replace('_',' ')); return; }
            else detail.add(line);
        }
        if (header == null) { System.out.println("Unexpected server response."); return; }
        ...
        System.out.printf("%-8s %-14s %6s %10s %10s %10s %10s%n",
                "SKU", "Category", "Qty", "Unit", "Base", "Discount", "Line Total");
        System.out.println("-".repeat(86));
        for (String ln : detail) {
            ...
            } else if (ln.startsWith("GIFT_SHIRT")) {
                // already indicated by header, but keep for completeness
            }
        }
        if (gift) {
            System.out.println("-".repeat(86));
    ...
    private void adminLogsMenu() {
        while (true) {
            ...
            String path = null;
            switch (c) {
                case "1": path = "logs/employees.log"; break;
                case "2": path = "logs/customers.log"; break;
                case "3": path = "logs/transactions.log"; break;
                case "4": path = "logs/auth.log"; break;
                case "5": path = "logs/chat_messages.csv"; break;
                default:
                    System.out.println("Invalid choice.");
                    continue;
            }
    ...
    private String passwordPolicyError(String pwd) {
        if (pwd == null || pwd.isEmpty()) return "Password is empty";
        if (pwd.length() < PasswordPolicy.minimumLength())
            return "Too short (min " + PasswordPolicy.minimumLength() + ")";
        // Use 'requireDigit' flag as "require special char"
        if (PasswordPolicy.requireDigit() && !pwd.matches(".*[^A-Za-z0-9].*"))
            return "Must contain at least one special char (e.g. !@#$%)";
        if (PasswordPolicy.requireLetter() && !pwd.matches(".*[A-Za-z].*"))
