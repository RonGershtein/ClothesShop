// ============================================================================
// Simple TCP chat server. Manages logins, pairing requests, conversations,
// and messaging between employees and managers across branches.
// ============================================================================
...
public class ChatServer {
    ...
    // ========= session =========
    // Holds per-connection state: socket, IO streams, user identity and status.
    private static final class Session {
        ...
        // --------------------------------------------------------------------
        // Session(): binds streams and stores user identity.
        // Does not do any network read/write here.
        // --------------------------------------------------------------------
        ...
        // --------------------------------------------------------------------
        // send(): convenience to write a single line to the client.
        // Auto-flush is enabled on the writer.
        // --------------------------------------------------------------------
        void send(String line) { out.println(line); }

        // --------------------------------------------------------------------
        // closeQuietly(): closes the socket without throwing checked exceptions.
        // Useful during cleanup.
        // --------------------------------------------------------------------
        void closeQuietly() { try { sock.close(); } catch (IOException ignored) {} }

        // --------------------------------------------------------------------
        // isManager(): returns true if this user is a shift manager.
        // Used to gate manager-only commands.
        // --------------------------------------------------------------------
        boolean isManager() { return "SHIFT_MANAGER".equalsIgnoreCase(role); }
    }

    // ========= conversation =========
    // A live chat room between 2+ members with a random short id.
    private static final class Conversation {
        final String id = UUID.randomUUID().toString().substring(0, 8);
        final Set<Session> members = Collections.newSetFromMap(new ConcurrentHashMap<Session, Boolean>());

        // --------------------------------------------------------------------
        // add(): adds a member, marks them as busy, and assigns the conversation id.
        // --------------------------------------------------------------------
        void add(Session s) { members.add(s); s.conversationId = id; s.busy = true; }

        // --------------------------------------------------------------------
        // remove(): removes a member and clears their busy/conversation flags.
        // --------------------------------------------------------------------
        void remove(Session s) { members.remove(s); s.conversationId = null; s.busy = false; }

        // --------------------------------------------------------------------
        // broadcast(): sends a line to all current members.
        // --------------------------------------------------------------------
        ...
        // --------------------------------------------------------------------
        // participantsCsv(): returns "user1,user2,..." for display/logging.
        // --------------------------------------------------------------------
    ...
    // ========= broadcast request =========
    // Represents an open pairing request that idle users can accept.
    private static final class BroadcastRequest {
        ...
        // --------------------------------------------------------------------
        // BroadcastRequest(): stores who requested and the target branch filter.
        // --------------------------------------------------------------------
    ...
    // ------------------------------------------------------------------------
    // ChatServer(): stores the TCP port to listen on.
    // ------------------------------------------------------------------------
    public ChatServer(int port) { this.port = port; }

    // ------------------------------------------------------------------------
    // start(): main server loop.
    // Binds a ServerSocket and accepts clients, each served on the thread pool.
    // ------------------------------------------------------------------------
    public void start() {
    ...
    // ------------------------------------------------------------------------
    // serve(): handles a single client connection lifecycle.
    // Validates HELLO, registers the user, then reads commands until disconnect.
    // ------------------------------------------------------------------------
    ...
    // ------------------------------------------------------------------------
    // handle(): parses one incoming line and routes to the correct command.
    // Supports request/accept/join/list/msg/end/quit.
    // ------------------------------------------------------------------------
    ...
    // ------------------------------------------------------------------------
    // requestBranch(): broadcasts a request to idle users in a specific branch.
    // The first acceptor will be paired with the requester.
    // ------------------------------------------------------------------------
    ...
    // ------------------------------------------------------------------------
    // requestUser(): tries to start a conversation with a specific user.
    // If unavailable, records a missed request for callback.
    // ------------------------------------------------------------------------
    ...
    // ------------------------------------------------------------------------
    // acceptRequest(): accepts a broadcast request by id if eligible and free.
    // Pairs acceptor with requester and closes the request for others.
    // ------------------------------------------------------------------------
    ...
    // ------------------------------------------------------------------------
    // listConvs(): for managers only, lists all active conversations.
    // Sends "CONV <id> <participants>" lines and then "OK END".
    // ------------------------------------------------------------------------
    ...
    // ------------------------------------------------------------------------
    // joinConv(): allows a manager to join a specific conversation by id.
    // Adds the manager as a member and notifies participants.
    // ------------------------------------------------------------------------
    ...
    // ------------------------------------------------------------------------
    // doCallback(): tries to call back a user who previously requested you.
    // Starts a conversation if the target is online and idle.
    // ------------------------------------------------------------------------
    ...
    // ------------------------------------------------------------------------
    // doMsg(): delivers a text message to all members of the current conversation.
    // Validates that the sender is still in a live conversation.
    // ------------------------------------------------------------------------
    ...
    // ------------------------------------------------------------------------
    // endConversation(): leaves the current conversation.
    // If only one member remains, closes the conversation for everyone.
    // ------------------------------------------------------------------------
    ...
    // ------------------------------------------------------------------------
    // startConversation(): creates a new conversation between two idle users.
    // Marks both as busy and notifies them with a PAIRED line.
    // ------------------------------------------------------------------------
    ...
    // ------------------------------------------------------------------------
    // notifyIncoming(): informs an eligible idle candidate about a new request.
    // Skips requester themselves and ineligible branches.
    // ------------------------------------------------------------------------
    ...
    // ------------------------------------------------------------------------
    // notifyRequestTaken(): tells all notified candidates that the request is taken.
    // Clears the notified set afterwards.
    // ------------------------------------------------------------------------
    ...
    // ------------------------------------------------------------------------
    // cancelRequest(): cancels a request and notifies all notified candidates.
    // Removes it from the openRequests map.
    // ------------------------------------------------------------------------
    ...
    // ------------------------------------------------------------------------
    // deliverOpenRequestsToIdle(): when a user becomes idle, send them eligible requests.
    // Iterates current open requests and notifies if applicable.
    // ------------------------------------------------------------------------
    ...
    // ------------------------------------------------------------------------
    // markIdle(): adds a session to the idle set of its branch.
    // Used to find candidates for requests.
    // ------------------------------------------------------------------------
    ...
    // ------------------------------------------------------------------------
    // unmarkIdle(): removes a session from the idle set of its branch.
    ...
    // ------------------------------------------------------------------------
    // ensureNotBusy(): throws if the session is already in a conversation.
    // Guards commands that require the user to be idle.
    // ------------------------------------------------------------------------
    ...
    // ------------------------------------------------------------------------
    // flushMissed(): sends any stored "missed request" notifications to the user.
    // Clears the stored list after delivering.
    // ------------------------------------------------------------------------
    ...
    // ------------------------------------------------------------------------
    // cleanupSession(): full cleanup on disconnect.
    // Logs, cancels open requests by this user, leaves conversations, and removes from maps.
    // ------------------------------------------------------------------------
    ...
    // ------------------------------------------------------------------------
    // closeAndCleanup(): wrapper to run cleanup and then close the socket.
    // Used when processing QUIT.
    // ------------------------------------------------------------------------
    ...
    // ------------------------------------------------------------------------
    // main(): starts the server on default port 6060 or a port from args[0].
    // Also installs a shutdown hook to log stop events.
    // ------------------------------------------------------------------------
