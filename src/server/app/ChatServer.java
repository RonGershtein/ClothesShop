...
public class ChatServer {
    ...
    private void handle(Session s, String line) {
        try {
            ...
            if ("REQUEST_ANY_OTHER_BRANCH".equals(cmd)) {
                requestAnyOtherBranch(s);
            } else if ("REQUEST_BRANCH".equals(cmd)) {
                requestBranch(s, rest);
            } else if ("REQUEST_USER".equals(cmd)) {
                requestUser(s, rest);
            } else if ("ACCEPT".equals(cmd)) {
                acceptRequest(s, rest);
            } else if ("LIST_CONVS".equals(cmd)) {
                listConvs(s);
            } else if ("JOIN".equals(cmd)) {
                joinConv(s, rest);
            } else if ("CALLBACK".equals(cmd)) {
                doCallback(s, rest);
