                ...
                if ("LOGIN".equals(cmd)) { // LOGIN <username> <password> <role: employee|admin>
                    ...
                    if (spec != null && !spec.isBlank()) {
                        for (String token : spec.split(",")) {
                            ...
                            String sku = kv[0];
                            int qty;
                            try { qty = Integer.parseInt(kv[1]); } catch (NumberFormatException nfe) { out.println("ERR BAD_QTY"); cart.clear(); break; }
                            if (qty <= 0) { out.println("ERR BAD_QTY"); cart.clear(); break; }

                            Product p = inventory.findProduct(branch, sku)
                                    .orElseThrow(() -> new IllegalStateException("SKU not found in branch: " + sku));
                            Line ln = new Line();
                            ln.sku = sku; ln.qty = qty; ln.product = p;
                            cart.add(ln);
                        }
                    }
                    if (cart.isEmpty()) { out.println("ERR EMPTY_CART"); continue; }
                    ...
                    // Absolute discount from customer strategy (e.g., VIP 12%)
                    BigDecimal discountAbs = customer.type().applyDiscount(baseTotal);
                    if (discountAbs.compareTo(baseTotal) > 0) discountAbs = baseTotal; // guard
                    BigDecimal finalTotal = baseTotal.subtract(discountAbs);

                    // Gift rule: if final total >= 300 -> gift shirt
                    boolean gift = finalTotal.compareTo(new BigDecimal("300")) >= 0;

                    // Update stock (commit), one line at a time
                    for (Line ln : cart) {
                        inventory.updateQuantity(branch, ln.sku, -ln.qty);
                    }

                    // Record single purchase (for tiering)
                    customers.recordPurchase(customerId);

                    // Log sale summary
                    Loggers.transactions().info(String.format(
                            "SALE_MULTI: Branch=%s, Customer=%s, Type=%s, Items=%d, Base=%s, Discount=%s, Final=%s, Gift=%s",
                            branch.name(), customerId, customer.type().code(), cart.size(),
                            baseTotal.toPlainString(), discountAbs.toPlainString(), finalTotal.toPlainString(), gift));
                    ...
                    if (gift) hdr.append(" GIFT");
                    out.println(hdr.toString());

                    // Per-line breakdown (proportional discount)
                    java.math.BigDecimal rate = (baseTotal.signum() == 0)
                            ? BigDecimal.ZERO
